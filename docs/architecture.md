...
- **Custodia de USDC:** el contrato toma custodia total del USDC en `init` (vía `TokenClient::transfer` del Stellar Asset Contract) y lo distribuye en `release_phase`. El USDC es un *Stellar Asset* expuesto a Soroban a través del SAC.
- **Autorización explícita:** `require_auth()` valida que el exportador firme `init`/`release_phase` y que el admin/oráculo firme `trigger_disaster` y `set_usdc`.
...
Tres fuentes, todas por software (cero hardware en el MVP):
...
El oráculo (admin del contrato) firma y envía `trigger_disaster` on-chain cuando se detecta el umbral paramétrico. Las lecturas se persisten en `weather_readings` e `iot_readings` para auditoría. Ver [`../web/app/api/data/weather/route.ts`](../web/app/api/data/weather/route.ts) y [`../web/app/api/data/iot/route.ts`](../web/app/api/data/iot/route.ts).

## 4. Capa Datos — Supabase (Postgres + Auth)

- **Postgres** persiste perfiles, cultivos, presupuesto por fases, el escrow off-chain, el *ledger* de tx hashes y las lecturas de los oráculos.
...
El agricultor nunca ve Stellar ni USDC. Tres opciones progresivas:

1. **Widget Transak/MoonPay** (MVP hackathon) — botón *"Retirar a Nequi"* en el frontend.
...
Para una liberación de fase, el flujo es:
...
2. El backend persiste `(contract_id, phase_number, tx_hash, amount_released)` en `phase_ledger`.
3. El dashboard consulta `phase_ledger` + `get_escrow_state` y actualiza la timeline.

Para un desastre, el oráculo (admin) ejecuta el mismo patrón pero invocando `trigger_disaster` y luego `contracts.update(status='frozen')`. El "wow moment" del frontend anima la transición.
