...
> Factoring agrícola paramétrico por ciclo en Stellar. Un smart contract en Soroban actúa como escrow por fases del ciclo agronómico entre un exportador y un agricultor, con seguro climático automático: si un oráculo detecta helada o sequía, el contrato reestructura la deuda y congela los fondos en tiempo real, sin intermediarios.

> 🌐 **Idiomas / Languages:** [English](./README.en.md) · **Español** (este archivo)
...
**TL;DR (60 segundos).** El agro latinoamericano pierde miles de millones al año porque los bancos no prestan a pequeños agricultores por el riesgo climático, y cuando lo hacen mediante factoring tradicional, entregan todo el dinero de golpe sin saber si se usó correctamente. Construimos en Stellar el primer **AgroFactoring Paramétrico por Ciclo**: un exportador financia la siembra mediante un smart contract que actúa como escrow inteligente. El agricultor no recibe todo el dinero de golpe; recibe liquidez inmediata solo para la primera fase de su ciclo agronómico. Para recibir el dinero de la siguiente fase, el contrato verifica vía satélite que la fase anterior se completó. Si un oráculo climático detecta helada o sequía, el contrato automáticamente congela los fondos del escrow para proteger al exportador, quema un porcentaje de la deuda y libera un fondo de rescate para que el agricultor sobreviva. Sin intermediarios, sin ajustadores de seguros y sin sensores físicos caros. Liquidez inmediata y segura por código en Stellar.
...
AgroFactoring es un sistema de **factoring agrícola paramétrico por ciclo** construido sobre Stellar. Un smart contract Soroban mantiene la custodia del capital (USDC) depositado por un exportador y lo libera al agricultor **fase por fase**, conforme avanza el ciclo agronómico (Adecuación → Siembra → Levante → Cosecha → Despacho). Un oráculo de software (APIs satelitales y climáticas, sin hardware en la finca) dispara dos eventos automáticos:

- **Verificación de fase:** el satélite confirma avance del cultivo (índice NDVI) → el contrato libera el siguiente tramo de USDC.
- **Seguro paramétrico:** el oráculo climático detecta helada (`temp < 2 °C` durante más de 4 h) o sequía → el contrato **congela** el escrow de fases futuras (protegiendo al exportador), **quema** un porcentaje de la deuda generada hasta ese momento y **libera** un fondo de rescate paramétrico para que el agricultor sobreviva la temporada.

El agricultor nunca ve Stellar ni USDC: retira a su cuenta bancaria colombiana (Nequi/PSE/ACH) a través de un gateway fiat (Transak para el MVP, Bitso API o un Anchor SEP-24 nativo en producción).
...
**Factoring agrícola por fases con seguro paramétrico incrustado en Stellar.** Se elimina la contradicción de *"dinero inmediato vs. escrow"* al liberar el capital estrictamente conforme avanza el ciclo agronómico:
...
2. **El escrow inteligente por fases.** El exportador deposita el USDC total en el smart contract Soroban. Solo se libera el primer tramo al instante; el resto queda bloqueado en escrow.
3. **El oráculo climático/agrícola.** Un backend Node.js consume APIs públicas vinculadas a las coordenadas GPS de la finca para verificar el estado del cultivo (imágenes satelitales NDVI) y el clima (OpenWeatherMap / IDEAM).
4. **Los escenarios.**
   - **Éxito por fase:** el oráculo satelital confirma avance → el contrato desbloquea la siguiente fase → al finalizar el Despacho el exportador recibe su pago con intereses.
   - **Desastre climático:** el oráculo detecta helada/sequía → el contrato congela el escrow de fases futuras, quema un % de la deuda generada y ejecuta el fondo de rescate paramétrico.
...
| Opción | Tiempo | Cómo |
|---|---|---|
| Widget fiat (hackathon MVP) | 2 h | Transak/MoonPay widget en el frontend: "Retirar a Nequi" convierte USDC → COP. |
...
1. **Capa Blockchain — Stellar / Soroban (Rust → WASM).** El smart contract `Agro_Factoring` mantiene la custodia del USDC y ejecuta la lógica de liberación por fases y la congelación paramétrica. Detalle completo en [`docs/smart-contract.md`](./docs/smart-contract.md).
...
3. **Capa Oráculos — Backend Node.js.** Consume OpenWeatherMap/IDEAM (clima) y Sentinel Hub (NDVI satelital) o evidencia fotográfica georeferenciada subida desde el celular del agricultor. El oráculo firma como *admin* del contrato para los triggers paramétricos.
4. **Capa Datos — Supabase (Postgres + Auth).** Persiste perfiles, cultivos, contratos, presupuesto por fases, el *ledger* de tx hashes on-chain y las lecturas meteorológicas/IoT. Detalle completo en [`docs/database.md`](./docs/database.md).
5. **Capa Fiat — última milla.** Transak (MVP) / Bitso API (v1) / Anchor SEP-24 (escala) para convertir USDC → COP en la cuenta del agricultor.

El diagrama de componentes completo está en [`docs/architecture.md`](./docs/architecture.md).

## 7. Smart Contract (Soroban)

El contrato `Agro_Factoring` está en [`Stellar/contracts/Agro_Factoring/src/lib.rs`](./Stellar/contracts/Agro_Factoring/src/lib.rs) y expone cuatro funciones:
...
| `release_phase(crop_id, phase_number)` | exportador | Libera un tramo al agricultor en orden estricto ascendente; al liberar la última fase el escrow pasa a `Completed`. |
| `trigger_disaster(exporter, farmer, crop_id)` | admin/oráculo | Congela el escrow: estado `Frozen`, sin más liberaciones. |
| `get_escrow_state(exporter, farmer, crop_id)` | partes | Lector de estado (requiere que las partes coincidan). |

Estados del escrow (enum `EscrowStatus`): `Active` → `Completed` (éxito) o `Active` → `Frozen` (desastre). La gestión de TTL (~30 días) evita el archivado. Diagrama de máquina de estados:
...
Siete tablas coordinan el estado *off-chain* que complementa al estado on-chain:
...
| `phase_ledger` | Auditoría: cada `release_phase` registra `tx_hash`, `phase_number`, `amount_released`. |
...
### Escenario A — Éxito por fase (liberación progresiva)

![Flujo de liberación por fase](./docs/images/flow-release-phase.png)

El exportador deposita el total → el contrato libera la fase 1 → el oráculo satelital confirma el NDVI de cada fase → el contrato libera la siguiente → al finalizar el Despacho el escrow pasa a `Completed` y el exportador recibe su pago con intereses. Cada `release_phase` queda registrado en `phase_ledger` con su `tx_hash`.

### Escenario B — Desastre climático (el "wow moment")

![Flujo de desastre climático](./docs/images/flow-disaster.png)

El agricultor pulsa "Simular Desastre Climático" → el backend detecta `temp < 2 °C` durante > 4 h → el oráculo (admin) firma y envía `trigger_disaster` on-chain → el contrato **congela** las fases 3, 4 y 5 (protegiendo $3.000 del exportador), **quema** el 30 % de la deuda generada (de $1.000 a $700) y **libera** el Fondo de Rescate Paramétrico ($300 USDC) para que el agricultor sobreviva. El impacto es simultáneo en ambas vistas (exportador y agricultor).
...
|---|---|
| Smart contract | Soroban (Rust/WASM) — lógica de estado por fases |
...
| Autenticación | `jose` (JWT) sobre Supabase Auth |
| Base de datos | Supabase (Postgres + Auth) |
| Stablecoin | USDC en Stellar testnet |
| Fiat off-ramp | Transak widget (hackathon) / Bitso API (prod v1) / Anchor SEP-24 (escala) |
...
- Una cuenta en **Stellar testnet** fondeada vía Friendbot, con un trustline a USDC testnet para el exportador y el agricultor.
...
# ejecutar los tests (con snapshots)
cargo test
...
> Las unidades de USDC usan 7 decimales en Stellar, por eso `5000 USDC` = `5000000000` stroops. Los 22 snapshots de tests bajo `Stellar/contracts/Agro_Factoring/test_snapshots/` documentan el comportamiento esperado de `init`, `release_phase` y `trigger_disaster` (escritura por `cargo test`, difs auditables).
...
Credenciales de prueba del hackathon:
...
Las migraciones ya están aplicadas en el proyecto Supabase vinculado (9 migraciones listadas en [`docs/database.md`](./docs/database.md)). Para reproducir el esquema en un proyecto nuevo, aplica las migraciones en orden desde el panel de Supabase o con la CLI `supabase db push`. El seed de demo incluye un cultivo (Café Caturra en Salento, Quindío), su presupuesto por 5 fases y el contrato demo.

## 13. Roadmap

### Etapa actual — MVP del hackathon (oráculos por API)

- Smart contract Soroban operativo en testnet: `init`, `release_phase`, `trigger_disaster`, `get_escrow_state`.
...
- Última milla fiat vía **widget** (Transak) y ledger de tx hashes en Supabase.
